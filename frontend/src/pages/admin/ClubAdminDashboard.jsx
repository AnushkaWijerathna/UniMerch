...
function Toast({ message, type = "success" }) {
  ...
  return (
    <div
      className={`fixed top-4 right-4 z-50 px-5 py-3 rounded-xl shadow-lg text-sm font-medium border
      ${type === "success"
          ? "bg-white border-green-200 text-green-700"
          : "bg-white border-red-200 text-red-600"
        }`}
...
export default function ClubAdminDashboard() {
 ...
 const clubMongoId = localStorage.getItem("clubMongoId");
  ...
  const fetchOrders = async () => {
    try {
      const res = await axios.get(`http://localhost:8080/api/orders/club/${clubMongoId}`);
      setOrders(res.data);
    } catch (err) {
      console.log(err);
    }
  ...
  const fetchProducts = async () => {
    try {
      const res = await axios.get(`http://localhost:8080/api/products/club/${clubMongoId}`);
  ...
  const fetchStats = async () => {
    try {
      const res = await axios.get(`http://localhost:8080/api/reports/club/${clubMongoId}/stats`);
      setStats(res.data);
    } catch (err) {
      console.log(err);
    }
  ...
  try {
    const res = await axios.get(
      `http://localhost:8080/api/clubs/${clubMongoId}`
    );
  ...
  try {
    ...
    if (!clubMongoId) {
      ...
      // save BOTH ids
      localStorage.setItem("clubMongoId", res.data.mongoId);
  ...
  return (
    <div className="min-h-screen bg-[#FBF5E6]">
      ...
      <nav className="bg-white shadow-sm px-6 py-4 flex justify-between">
        <h1 className="font-bold text-[#3D0C0C]">Club Admin Dashboard</h1>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white p-4 rounded-xl">
            <p>Total Revenue</p>
            <h2>${stats.tRevenue}</h2>
          </div>
          <div className="bg-white p-4 rounded-xl">
            <p>Pending</p>
            <h2>{stats.orderPending}</h2>
          </div>
          <div className="bg-white p-4 rounded-xl">
            <p>Completed</p>
            <h2>{stats.orderCompleted}</h2>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-4">
          ...
          {/* SETTINGS (RESTORED FULL SECTION) */}
          {activeTab === "settings" && (
            <div className="space-y-4">
              ...
              <input
                ...
                value={settings.desc}
                onChange={(e) =>
                  setSettings({ ...settings, desc: e.target.value })
                }
                className="border p-2 w-full rounded"
              />
              ...
              <input
                ...
                value={settings.pickupLocation}
                onChange={(e) =>
                  setSettings({ ...settings, pickupLocation: e.target.value })
                }
                className="border p-2 w-full rounded"
              />
              ...
              <select
                value={settings.pickupDay}
                onChange={(e) =>
                  setSettings({ ...settings, pickupDay: e.target.value })
                }
                className="border p-2 w-full rounded"
              >
                {["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"].map(d => (
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings({ ...settings, secretaryEmail: e.target.value })
                }
                className="border p-2 w-full rounded"
              />
              ...
              <button
                ...
                className="bg-[#3D0C0C] text-white px-4 py-2 rounded"
              >
                Save Settings
              </button>
            </div>
